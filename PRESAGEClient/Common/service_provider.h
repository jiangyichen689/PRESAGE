...
typedef struct trust_ps_sec_prop_desc_t
{
    uint8_t  sample_ps_sec_prop_desc[256];
} trust_ps_sec_prop_desc_t;
...
typedef struct sample_ra_msg2_t
{
    sample_ec_pub_t             g_b;        // the Endian-ness of Gb is
                                            // Little-Endian
    sample_spid_t               spid;
    uint16_t					 quote_type; /* unlinkable Quote(0) or linkable Quote(0) in little endian*///New by wwj
    ...
    uint16_t					 kdf_id;	 /* key derivation function id in little endian. 
                                             0x0001 for AES-CMAC Entropy Extraction and Key Derivation *///New by wwj
