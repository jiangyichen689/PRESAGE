...
cmph_t *fch_new(cmph_config_t *mph, double c)
{
	...
	cmph_uint32 iterations = 500;//modified by cf, original is 100
...
void fch_load(FILE *f, cmph_t *mphf)
{
	...
	printf("Hash state of h1 has %u bytes\n", buflen);
	...
	printf("Hash state of h2 has %u bytes\n", buflen);
	buf = (char *)malloc((size_t)buflen);
	nbytes = fread(buf, (size_t)buflen, (size_t)1, f);
