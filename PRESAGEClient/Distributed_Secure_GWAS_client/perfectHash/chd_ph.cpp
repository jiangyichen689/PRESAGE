...
{
	double r = _r, n = _n;
	return (1 + (r/n - 1.0 + 1.0/(2.0*n))*log(1 - n/r))/log(2.0);
};
...
static inline double chd_ph_get_entropy(cmph_uint32 * disp_table, cmph_uint32 n, cmph_uint32 max_probes)
{
	...
	{
		if(probe_counts[i] > 0)
			entropy -= probe_counts[i]*log((double)probe_counts[i]/(double)n)/log(2.0);
	};
	free(probe_counts);
	return entropy;
};
...
cmph_t *chd_ph_new(cmph_config_t *mph, double c)
{
	...
	max_probes = (cmph_uint32)(((log((double)chd_ph->m)/log(2.0))/20) * max_probes);
